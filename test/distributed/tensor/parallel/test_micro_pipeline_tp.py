...
class MicroPipelineTPTest(TestCase):
    ...
    def test_find_all_gather_patterns(self):
        ...
        def func(A_shard: torch.Tensor) -> torch.Tensor:
            x = all_gather_tensor(A_shard, gather_dim=0, group=group.group_name)
            y = all_gather_tensor(A_shard, gather_dim=1, group=group.group_name)
            return x, y

        A_shard = torch.rand(64, 32, device="cuda")

        gm = make_fx(func)(A_shard)
    ...
    def test_find_reduce_scatter_patterns(self):
        ...
        def func(A_shard: torch.Tensor) -> torch.Tensor:
            x = reduce_scatter_tensor(
                A_shard, "sum", scatter_dim=0, group=group.group_name
            )
            y = reduce_scatter_tensor(
                A_shard, "avg", scatter_dim=1, group=group.group_name
            )
            return x, y
