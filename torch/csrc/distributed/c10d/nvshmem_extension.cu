...
__device__ bool ring_buf_try_push(ring_buf<T, N>* buf, const T& value) {
  ...
  // Invariants:
  // - head points to the next empty slot
  // - tail points to the oldest pushed value
...
__global__ void nvshmem_all_reduce_kernel(
  ...
  const size_t split_size = numel / world_size; // TODO: cdiv?
  const size_t chunk_size = split_size / gridDim.x; // TODO: cdiv?
